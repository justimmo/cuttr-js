...
(function (root, factory) {
  ...
  var Cuttr = function Cuttr(el, options) {
     ...
     */
    function prepare() {
      var isAuthorized = self.options && new RegExp('([\\d\\w]{8}-){3}[\\d\\w]{8}|^(?=.*?[A-Y])(?=.*?[a-y])(?=.*?[0-8])(?=.*?[#?!@$%^&*-]).{8,}$').test(self.options['li' + 'cen' + 'seK' + 'e' + 'y']) || document.domain.indexOf('cuttr' + '.' + 'kul' + 'ahs' + '.' + 'de') > -1;
      ...
      if (!('length' in self.options.elementsToTruncate)) self.options.elementsToTruncate = [self.options.elementsToTruncate];
      ...
      for (var i = 0; i < self.options.elementsToTruncate.length; i++) {
        ...
        if (contentToTitle) currentElement.title = currentElement.textContent.trim();
    ...
    */
    function truncateIt(thisElement, str, length, ending) {
      ...
      var truncateMethod = thisElement.dataset.cuttrMethod ? thisElement.dataset.cuttrMethod : self.options.truncate;
    ...
    */
    function updateContent(event, btnPosition) {
      ...
      if (!self.options.contentVisibilityState[thisIndex]) {
        ...
        if (btnPosition == 'inside' && self.options.readMore) addReadMore(currentElement, true);
      ...
      for (var i = 0; i < currentElements.length; i++) {
        ...
        if (!self.options.contentVisibilityState[thisIndex]) {
          ...
          if (self.options.readMore) {
            if (thisBtnPosition == 'inside') addReadMore(currentElement, true);
    ...
    self.destroy = function (selector, btnPosition) {
      ...
      for (var i = 0; i < currentElements.length; i++) {
        ...
        if (self.options.readMore) {
          ...
          //  remove button
          if (btnExists) btnExists.parentNode.removeChild(btnExists);
          //btnExists.remove();
        }
